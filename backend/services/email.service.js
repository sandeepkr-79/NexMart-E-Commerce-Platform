...
const isEmailConfigured = process.env.NODEMAILER_USER && process.env.NODEMAILER_PASS;
...
if (isEmailConfigured) {
  ...
  console.log('Nodemailer SMTP details missing. Emails & OTPs will log directly to console.');
}
...
export const sendOtpEmail = async (email, otp) => {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ddd; border-radius: 5px; max-width: 600px; color: #333;">
      <h2 style="color: #4a154b; text-align: center;">Welcome to NexMart!</h2>
      ...
      <p style="font-size: 11px; color: #999; text-align: center;">NexMart AI Multi-Vendor Marketplace, Inc.</p>
    </div>
  `;
  return await sendEmail({ to: email, subject, html, text: `Your NexMart OTP is ${otp}. Expires in 10 minutes.` });
};

export const sendSellerStatusEmail = async (email, shopName, status, feedback = '') => {
  const subject = `NexMart - Seller Application Update: ${status}`;
  const html = `
    <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
      <h2>Application Status: ${status}</h2>
      <p>Hello,</p>
      <p>Your application for shop <strong>${shopName}</strong> has been <strong>${status.toLowerCase()}</strong> by the platform administrator.</p>
      ${feedback ? `<p><strong>Feedback:</strong> ${feedback}</p>` : ''}
      <p>If you have any questions, please contact the NexMart admin team.</p>
    </div>
  `;
  return await sendEmail({ to: email, subject, html, text: `Your shop application status: ${status}. Feedback: ${feedback}` });
};
