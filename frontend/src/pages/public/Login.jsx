...
import { useLoginMutation, useVerifyOtpMutation, useGoogleLoginMutation } from '../../app/apiSlice.js';
...
const Login = () => {
  ...
  // OTP Verification state if login detects unverified status
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.data?.code === 'OTP_REQUIRED') {
        ...
        toast.success('Please verify your email. OTP sent.');
      } else {
    ...
    <div className="flex justify-center items-center min-h-[70vh] font-sans">
      <div className="card w-full max-w-md bg-slate-900 border border-slate-800 shadow-2xl rounded-2xl p-6 md:p-8 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400 font-medium">Log in to manage shop, buy, or chat with AI</p>
        </div>
        ...
        {!showOtpModal ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="divider text-[10px] text-slate-500 uppercase tracking-widest my-2">Or continue with</div>
            ...
            >
              <span className="font-extrabold text-violet-400">G</span> Sign in with Google
            </button>

            <div className="text-center text-xs text-slate-400 mt-4">
              Don't have an account?{' '}
              <Link to="/signup" className="text-violet-400 hover:underline font-semibold">Sign up</Link>
            </div>
          </form>
        ) : (
          <form onSubmit={handleOtpVerify} className="space-y-4">
            ...
            >
              {isVerifying ? <span className="loading loading-spinner"></span> : 'Verify & Log In'}
            </button>
